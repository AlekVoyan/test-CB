...
**Two changes are newer than the reported run.** The offered word (`didYouMean`, §2) and the widened phonetic check landed after the pass reported below. A targeted run over the sessions most at risk — the holdout, the not-found tests and the clarification pair — passed 100%, with some questions unscored because the free endpoint was rate-limiting by then (`eval/results/report-S1+S4+S6+S11+S14+S15.md`). A full pass was attempted twice more and both times the free endpoint refused most of it — 41 and then 49 of 111 questions came back `429`. What did get through the second attempt scored 98.1% factual and 100% citation with 0 critical failures across 52 scored runs, including every holdout question (`eval/results/report-partial-after-changes.md`), but a pass that scores half the suite is not an acceptance check and is not reported as one. A full pass is queued for when the endpoint recovers or, better, for Claude Haiku 4.5.
...
- **R1b is the weakest test in the set — it passed 4 of 9 runs across the three passes (1/3 in the reported one), and it is a P0 test.** "What is the limit?" is ambiguous in two ways at once: which model, and which of the manual's several limits. The contract says a clarifying question is about which model or product, and when the model asks that, the reply "Model B." is answered with 12 units. When it instead asks "which limit — load, reservoir capacity or temperature?", the reply names a model and settles nothing, so it asks again and the test fails. The failure is safe — it declines, cites nothing and invents nothing — but it is the criterion of ТЗ §7.8 that is missed. I tried twice to fix it in the prompt and reverted both, which is why the shipped prompt is the one that produced the numbers above:
  ...
  I did not touch `expected.json`: the expected answer was recorded before any run, and the model asking a different, also reasonable question does not make the recorded answer wrong. On Claude Haiku 4.5 this is the first test I would re-measure. **After the reported run** the choice of model moved into code instead (§2, `clarify.ts`): the clarifying question is asked by code, and "Model B." reaches the model already joined to "What is the limit?". It is covered by unit tests and not yet measured in a full pass.
- **The "I couldn't verify an answer in the uploaded documents." fallback is English in all three languages.** It is the sentence shown and spoken when two attempts fail validation, and a Russian or Ukrainian listener gets it in English. Fixed after the reported run: the sentence now follows the answer language, and the validator reads all three wordings as not-found. Not yet re-measured in a full pass.
