...
- **Long documents are searched by meaning too.** A document set within the 8k-token budget goes to the model whole. Above it a selection is sent, and search by words alone misses a passage that answers in other words or in another language. So the browser also runs multilingual-e5-small (intfloat, MIT) with ONNX Runtime Web: paragraphs are embedded once, each question becomes a vector, and the word and meaning rankings are fused. The selection is held to 1,300 estimated tokens, so a question costs no more than a word-only selection. The model is a 135 MB download the first time a long document is loaded and comes from the browser's cache after that. Until a document is indexed (16 s for an 8-page magazine article), its questions are searched by words.
...
| `src/core/pdf.ts` | pdf.js text extraction, line grouping, columns (and columns inside a section) read in reading order, re-joining lines wrapped at the margin, and words hyphenated across them |
...
| `src/core/retriever.ts` | Tokenizer (keeps "Model A"), BM25, `full` / `topk` / `hybrid` evidence selection, closest passages |
| `src/core/semantic.ts` | Search by meaning, the arithmetic: passages (windows of lines within a paragraph), paragraph similarity, reciprocal rank fusion |
...
| `src/web/deviceVoice.ts`, `supertonic.worker.ts`, `supertonicText.ts` | On-device voice. The worker downloads Supertonic 3 from Hugging Face (a pinned revision) into Cache Storage, runs it with ONNX Runtime Web on WebGPU (WebAssembly as the last resort) and returns audio sentence by sentence. The first piece of an answer is short, and each kind of piece has a fixed input shape warmed up at load, so on WebGPU the first sound is ready in under a second and never waits for kernels to be built. `supertonicText.ts` is the text preparation, ported from Supertonic's MIT-licensed example. `speechNumbers.ts` says numbers, units, signs and codes as Russian or Ukrainian words before this voice reads them; the answer on screen keeps its digits |
| `src/web/embedder.ts`, `embedder.worker.ts`, `modelFiles.ts` | Search by meaning in the browser. The worker downloads multilingual-e5-small (a pinned revision) into Cache Storage, tokenizes with `@huggingface/tokenizers`, runs the model with ONNX Runtime Web on WebAssembly and returns unit-length vectors, in batches of passages of similar length. `modelFiles.ts` is the cached download both workers use |
