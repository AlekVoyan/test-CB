...
| Cold start of the deployed function | Vercel, `GET /api/answer` 8 minutes after the last request, then twice more | 271 ms, then 339 ms and 249 ms. Either the instance was still warm or its boot fits inside that range; a figure after hours of idleness is still **TBD** |
...
- **Answer language drift (not fixed).** A Ukrainian question asked right after a Russian one got a Russian answer (Nemotron). A check on the answer's alphabet (і ї є ґ against ы э ъ) with a retry would catch it.
