...
- **Don't** move a tab sideways, park it above the front tab's top line, or recolour it when another folder comes forward: depth shows in width, tab height and edge, and the colour belongs to the folder. Under the pointer a folder lifts whole and settles back, which is the one time a tab leaves the line.
