...
- **Question:** `input_tokens × $1/1M + output_tokens × $5/1M`, summed over all attempts (retries included, because each retry is billed), `+ spoken_characters / 1000 × $0.05` for the voice (the answer plus the Why line of an inferred answer; $0 when the browser's voice speaks).
