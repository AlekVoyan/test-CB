...
| A document says what it can be asked about | The sample manual has three questions written for it; any other document had none, so the first question was a guess. Its own headings — short lines of more than one word that do not end like a sentence — are offered instead, turned into questions in the answer's language. A document with no headings offers nothing: a sentence pulled from the middle reads as noise rather than a topic. | Built from the text at ingestion, so it costs nothing and works offline. On a scanned magazine the headings it finds can be front matter ("MIDSUMMER VOLUME") — true to the document, and only a suggestion either way. |
| A second pass at the evidence, when only a selection is sent | A loose question — "what is it good for", "where does the gas come from" — shares almost no words with the text that answers it, and on a document too large to send whole that is the difference between an answer and a refusal. The lines the first pass did reach lend their rarest words back to the query, and the second pass reaches what the first missed. Classic pseudo-relevance feedback, arithmetic on the index, no model call. It runs only when the corpus does not fit the prompt: when everything is sent, ranking only decides the order. | Six borrowed words can pull the ranking towards a paragraph that merely looks related. `selectEvidence(..., { feedback: false })` turns it off, so the two can be measured against each other on a larger corpus than the fixtures. |
| The model may offer the documents' word for what was asked about | The phonetic check catches a misheard word; it cannot catch a word that was heard correctly and is simply not what the documents call the thing. The model sees both, so the contract gained one optional field: on a not-found answer it may name the documents' own term ("водородные насосы" for "водородные опсо"), which then leads the suggestions under the refusal. It is checked before it is shown — a term no evidence line contains is dropped, silently and without a retry, and a term offered on an answer that did find something is dropped too. | One more field in the schema and three lines of prompt. It decorates a refusal, so a wrong offer costs nothing but a bad suggestion; the check keeps it to words the documents actually use. |
| A refusal offers what the document does have | "Not in the document" is correct and useless on its own: the reader is left guessing what else to ask. The lines that came closest are already ranked, so their opening words — the document's own wording, never a paraphrase — are offered as up to three questions to ask next, in the answer's language. Tapping one asks it. A scrap that starts mid-sentence, a figure legend ("10. Решетка 11. Ось") and a run of model codes are not offered; if nothing reads well, the refusal stands alone as before. | Costs nothing: the ranking is the retriever's, already computed. The suggestions are not spoken — the hosted voice may only speak text the server signed, and these are chosen in the browser. |
...
| Which model a question is about, settled in code | R1 and R1b were the weakest pair in the eval (§13): the model sometimes asked which limit rather than which model, and then could not connect the reply "Model B." to its own question. Two prompt fixes each traded one test for the other. `clarify.ts` now asks "Which model do you mean — Model A or Model B?" itself when the question names no model, the conversation has not settled one, and the closest lines — or the section under the closest heading — say different things for different models. A short reply that names a model, or one of the options offered, is joined to the question it answers ("What is the limit — Model B?") before anything is sent. | Only models a document writes as "Model X" are recognised: the holdout's "Nimbus Pro" is not, so that document still relies on the model. Matching is by shared word starts, not meaning. The question asked back is spoken by the browser's voice, like the mishearing check's, because no server call signs it. The eval runs the same code: in the reported run (§8) R1, R1b and X5 pass 3/3. |
...
Reported run: commit `12a9764`, NVIDIA Nemotron 3 Super, 37 tests, 3 runs per session, 111 questions, none unscored — the first full pass after the choice of model moved into code (§2) and the fallback sentence learned the answer's language, and the seventh full pass overall. The pass reported before it (`f3e1daf`) is kept in `eval/results/report-before-clarify.md`, others in `eval/results/report-pass-b.md` and in git history, and §9 carries the spread between them, which is wider than I expected. Verbatim answers and quotes for every run are in `eval/results/report.md`, raw records in `eval/results/actual-results.json`.
...
| M6 | replacement (v1 → v2) | What is the maximum for Model A? | answered · 24 units, not 20 · v2 p.2 | 3/3 (the answer changed from the v1 baseline in every run) |
| R1 | ambiguity | What is the limit? | needs_clarification · names Model A and Model B | 3/3, asked back by code (`clarify.ts`); 1/3 to 3/3 in the passes before it, when the model asked |
| R1b | spoken clarification | Model B. | answered · 12 units · v1 p.2 | 3/3, the reply joined to its question by code; 0/3 to 3/3 before — the weakest test in the set until then, see §13 |
...
| A6 | exception reasoning | Can Model B run at 15 units when it's 25°C? | answered · No (exception only below 20°C) · v1 p.3 | 1/3; 1/3 to 3/3 across the passes before |
...
| X5 | slip, model not named (guard) | How often should I clean the nozzel? | needs_clarification · names Model A and Model B | 3/3, asked back by code |
| L1–L6 | Russian and Ukrainian | facts, follow-up, exception, absent facts | answer in the selected language, quotes in English | 16/18 (L5, the Ukrainian exception question, 1/3; it has swung between 3/3 and 1/3 across passes) |
...
| The model reads before it decides | `627aaf7` | 96.7% (R1b 2/3) | 92.6% (A6 1/3) | 100% | 94.4% (L5 2/3) | 80.0% (X3) | 0 |
| **Reported**: which model, settled in code (five more passes between, in §9) | `12a9764` | **100%** | **92.6%** (A6 1/3) | **100%** | 91.7% (L5 1/3) | **80.0%** (X3) | **0** |

**Think-better tests, reported run:**
...
- **X5**: 3/3, now asked back by code before the model is called. It was 1/3 once, when the model declined instead of asking which model.

Across the 111 answers of the reported run the model marked 16 as inferred, and none was wrong. The guards held: A4, M5 and R1 3/3.
...
**Every change is inside the reported run.** The offered word (`didYouMean`, §2), the widened phonetic check, the choice of model in code and the fallback sentence in the answer's language all landed before `12a9764`, and that pass had no provider errors. Before it, two full passes were refused most of their questions by the free endpoint — 41 and then 49 of 111 came back `429`. What got through the second scored 98.1% factual and 100% citation with 0 critical failures across 52 scored runs (`eval/results/report-partial-after-changes.md`); a pass that scores half the suite is not an acceptance check, and neither of the two is reported as one.
...
In this run the two scores are equal in every group: each failure is a decline or a withheld answer, and those fail both. In `6f9f76b` they differed where X5 declined instead of asking: factual 0, but citation 1, because a non-answer should cite nothing. The case the two scores are meant to separate — right fact, wrong citation — did occur (R1b and A7 in the `016ee32` run, R1b and L5 in `f3e1daf`), but the validator rejected those answers before they could be shown or scored as correct. In the reported run it rejected L5 twice for a number no cited line contains.
...
| Question total, text pipeline (retrieval + LLM incl. retries + validation) | Node, 111 questions, reported run `12a9764` | median 2333 ms, p90 5797 ms, max 45399 ms; first model call median 2312 ms. Six of the 111 were asked back by code in about a millisecond, which pulls the median down a little. The passes before it: medians 2264 to 2893 ms, maxima 13.6 s to 54.4 s. Under the rate limiting of the sixth pass the median went to 6364 ms. The endpoint's mood moves this more than anything in the code does |
...
Almost all of the question latency is the model call; retrieval and validation are ~1 ms, and the neural voice adds about a third of a second. Retry hints add a second call on 5.4% of questions in the reported run: 19% before the analysis-first change, 7% before the think-better mode. Most of a question's time is the free endpoint itself. Its calls ranged from 1.2 to 7 s within the same minute, and one took 29 s. These are measurements on NVIDIA's free endpoint, whose latency varies; Claude Haiku 4.5 numbers are **TBD**.
...
| Tokens per question | 2163 in / 159 out (mean, retries included) | measured, reported eval `12a9764`, 111 questions (the six asked back by code count as zero). Before the think-better mode: 1452 / 87 (the prompt is longer now, and each answer carries a private analysis) |
| Retries | 6 of 111 questions (5.4%) | measured; their tokens are included above. Across the passes: 1.8% to 5.6%. A retry is a second call, so it is the cost line that moves most between passes |
| LLM per question, Nemotron 3 Super | mean $0.00025, max $0.00061 | measured tokens × OpenRouter paid price ($0.085 / $0.40 per MTok) |
...
| One pass over the P0 tests | $0.00229 (Nemotron) | measured |
...
- **R1b was the weakest test in the set — it passed 4 of 9 runs across three passes, 0/3 in the pass reported before `clarify.ts`, and it is a P0 test. Fixed in code: 3/3 in the reported run, and R1 with it.** "What is the limit?" is ambiguous in two ways at once: which model, and which of the manual's several limits. The contract says a clarifying question is about which model or product, and when the model asks that, the reply "Model B." is answered with 12 units. When it instead asks "which limit — load, reservoir capacity or temperature?", the reply names a model and settles nothing, so it asks again and the test fails. The failure is safe — it declines, cites nothing and invents nothing — but it is the criterion of ТЗ §7.8 that is missed. I tried twice to fix it in the prompt and reverted both, which is why the shipped prompt is the one that produced the numbers above:
  ...
  I did not touch `expected.json`: the expected answer was recorded before any run, and the model asking a different, also reasonable question does not make the recorded answer wrong. On Claude Haiku 4.5 this is the first test I would re-measure. The fix that held was not in the prompt at all: the choice of model moved into code (§2, `clarify.ts`). The clarifying question is asked by code, and "Model B." reaches the model already joined to "What is the limit?". A targeted run over S6 and S28 (`eval/results/report-S6+S28.md`) and then the reported full pass both came back 3/3 for R1, R1b and X5.
- **The "I couldn't verify an answer in the uploaded documents." fallback was English in all three languages (fixed).** It is the sentence shown and spoken when two attempts fail validation, and a Russian or Ukrainian listener got it in English. It now follows the answer language, and the validator reads all three wordings as not-found. In the reported run it is what L5's two failed runs said — in Ukrainian.
- **A6 failed 3/3 until the analysis-first change, 1/3 after it** — "Can Model B run at 15 units when it's 25°C?" gets "The uploaded documents do not specify…" instead of "No, the exception applies only below 20°C". A safe failure (declines, invents nothing, 0 citations), not fixed by two general prompt changes. Not tuned further to avoid fitting the prompt to one test. In the run `627aaf7` the retry hint that quotes the model's own reason back to it did not change its mind either. It has since settled down without any change aimed at it: 3/3, 2/3 and 3/3 in the three latest passes — another sign that a single pass measures the endpoint's mood as much as the pipeline. 1/3 in the reported run. **TBD:** result on Claude Haiku 4.5.
...
- **X3** — Nemotron never offered a related line for the battery question (0/3). Still 0/3 in the reported run.
...
- **L5 swings.** The Ukrainian exception question passed 3/3 in the pass reported before `clarify.ts`, 1/3 in a targeted re-run half an hour later on the same code, and 1/3 in the reported run. There both failures carried a number 6 that no cited line contains; the retry hint named the lines that do (the "6. Exceptions" heading and the filter line), the retry kept the number, and the answer was withheld. Rejected answers are not stored, so where the 6 came from is a guess — most likely the section number. Same cause as everything else in this section: one pass of this suite measures the endpoint's mood as much as the pipeline.
