...
| On-device voice: first download of the model (399 MB) | headless Chromium, my connection | about 35 s. Later loads come from the browser's cache: 4.9 s including a warm-up sentence |
| On-device voice: synthesis of one answer | same, WebGPU on Metal | 0.6–1.1 s for 40–140 characters (3.5–12 s of audio) once warm; 1.2–2.7 s for the first answers after loading, whose input lengths the session had not seen. WebAssembly: 5.5–11 s |
...
- The on-device voice needs WebGPU to be usable and downloads 399 MB once per browser. Its first answers after loading take longer, while WebGPU builds kernels for input lengths it has not seen. Its source repository is archived.
